 ...
 * <bitbar.desc>Displays a coloured dot for mouse, keyboard and laptop battery in the menu bar that changes from green to yellow to red the lower the power gets - based on the mouse-dot plugin by Manu Wallner (https://getbitbar.com/plugins/System/mouse-dot.1h.swift).
...
func mouseBattery() -> (c: String, s: String, a: String) {
    let commandString = "ioreg -n BNBMouseDevice | fgrep BatteryPercent | fgrep -v { | sed 's/[^[:digit:]]//g'"
...
var (c,s,a) = keyboardBattery()
powerStatus.keyboard.colorString = c
powerStatus.keyboard.dropdownString = s
powerStatus.keyboard.ansiColorString = a

(c,s,a) = mouseBattery()
powerStatus.mouse.colorString = c
powerStatus.mouse.dropdownString = s
powerStatus.mouse.ansiColorString = a

print("K: \u{001B}\(powerStatus.keyboard.ansiColorString)●\u{001B}[0m M: \u{001B}\(powerStatus.mouse.ansiColorString)●\u{001B}[0m| size=10 ansi=true")
print("---")
print("Keyboard:".padding(length: 11) + powerStatus.keyboard.dropdownString + "| font=Courier color=\(powerStatus.keyboard.colorString)")
print("Mouse:".padding(length: 11) + powerStatus.mouse.dropdownString + "| font=Courier color=\(powerStatus.mouse.colorString)")
